...
export default function HorizontalTVSection({ title, shows, seeAllHref }) {
  const list = shows ?? [];
  ...
  return (
    <section className="page-container py-8 sm:py-10">
      ...
      <div className="flex gap-3 sm:gap-4 overflow-x-auto pb-3 hide-scrollbar snap-x snap-mandatory -mx-4 px-4 sm:mx-0 sm:px-0">
        {list.slice(0, 14).map((show) => (
          <div key={show.id} className="w-[130px] sm:w-[148px] shrink-0 snap-start">
            <TVCard show={show} />
          </div>
        ))}
      </div>
